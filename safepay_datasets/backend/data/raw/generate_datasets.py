"""
SafePay AI — Dataset Generator
Run: python generate_datasets.py
Outputs: sms_spam.csv, creditcard.csv  (in this folder)
         ../processed/sms_cleaned.csv, ../processed/txn_engineered.csv
"""
...
# ── CONFIG ────────────────────────────────────────────
SMS_ROWS   = 1500   # increase for more data
TXN_ROWS   = 50000  # increase for more data
...
def gen_spam():
    ...
    return t.format(
        url=random.choice(["bit.ly/clm9x","tinyurl.com/win99","t.co/fraud123","sbi-verify.net","hdfc-kyc.in"]),
...
rows = [(gen_ham(),"ham") for _ in range(n_ham)] + \
       [(gen_spam(),"spam") for _ in range(n_spam)]
random.shuffle(rows)
...
print(f"sms_cleaned.csv    -> {len(sms_clean)} rows (processed)")

# ═══════════════════════════════
# 2. TRANSACTION / CREDIT CARD DATASET
# ═══════════════════════════════
n = TXN_ROWS
...
V1 = np.empty(n); V2 = np.empty(n); Amount = np.empty(n)
...
Amount[fidx] = np.concatenate([
    np.random.uniform(50000, 200000, half),
    np.random.uniform(1, 500, n_fraud - half)
])
Amount[lidx] = np.clip(np.random.exponential(3000, n_legit), 1, 100000)

cc = pd.DataFrame({
    "Time": np.arange(n), "V1": V1, "V2": V2,
    "V3": np.random.normal(0,1,n), "Amount": Amount, "Class": fraud_mask
})
...
eng[['amount','is_new_receiver','transactions_today','Class']]\
   .to_csv("../processed/txn_engineered.csv", index=False)
print(f"txn_engineered.csv -> {len(eng)} rows (processed)")
